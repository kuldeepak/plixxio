...
async function loadConfiguration(productId) {
    try {
        const response = await fetch(`https://relatives-chief-specialty-book.trycloudflare.com/api/public/configurator/${productId}?v=${Date.now()}`,
            {
...
async function calculatePrice(productId, selections, measurements, quantity) {
    try {
        const response = await fetch(`https://relatives-chief-specialty-book.trycloudflare.com/api/public/calculate-price?v=${Date.now()}`, {
            method: 'POST',
            cache: 'no-store', // 🚫 disable cache
            headers: {
